...
class Problem:
    ...
    def __init__(self, initial, destination=None):
        """The constructor specifies the initial state, and possibly a destination
        state, if there is a unique destination.  Your subclass's constructor can add
        other arguments."""
        self.initial = initial; self.destination = destination
    ...
    def goal_test(self, state):
        """Return True if the state is a destination. The default method compares the
        state to self.destination, as specified in the constructor. Implement this
        method if checking against a single self.destination is not enough."""
...
class SimpleProblemSolvingAgent(agents.Agent):
    ...
    def __init__(self):
        ...
        def program(percept):
            ...
            if not seq:
                destination = self.formulate_goal(state)
                problem = self.formulate_problem(state, destination)
...
def tree_search(problem, fringe):
    """Search through the successors of a problem to find a destination.
...
def best_first_graph_search(problem, f):
    ...
    if f is a heuristic estimate to the destination, then we have greedy best
...
class GraphProblem(Problem):
    ...
    def __init__(self, initial, destination, graph):
        Problem.__init__(self, initial, destination)
        self.graph = graph
    ...
    def h(self, node):
        "h function is straight-line distance from a node's state to destination."
        locs = getattr(self.graph, 'locations', None)
        if locs:
            return int(distance(locs[node.state], locs[self.destination]))
        else:
            return infinity
...
class InstrumentedProblem(Problem):
    ...
    def goal_test(self, state):
        "Return true if the state is a destination."
        self.goal_tests += 1
